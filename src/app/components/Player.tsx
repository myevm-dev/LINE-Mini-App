...
export default function Player() {
  ...
  useEffect(() => {
    ...
    // target "arms down" angles
    const ARM_Z_UPPER = THREE.MathUtils.degToRad(70);  // ±70°
    const ARM_Z_LOWER = THREE.MathUtils.degToRad(10);  // ±10°
    ...
    // talking controller (visemes: Aa → Ih → Ou)
    let talking = false;
    let talkT = 0;
    ...
    // events (Chat.tsx will dispatch only AFTER message is shown)
    ...
    loader.load(
      ...
      (gltf: GLTF) => {
        ...
        renderer.setAnimationLoop(() => {
          ...
          // enforce "arms down" every frame (beats any rig reset)
          if (LUp) LUp.rotation.set(0, 0, -ARM_Z_UPPER);
          if (LLo) LLo.rotation.set(0, 0, -ARM_Z_LOWER);
          if (RUp) RUp.rotation.set(0, 0,  ARM_Z_UPPER);
          if (RLo) RLo.rotation.set(0, 0,  ARM_Z_LOWER);
